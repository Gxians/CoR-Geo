...
def validate_experiment_config(config: ExperimentConfig) -> None:
    ...
    geometry = (
        ...
        int(distributed["per_gpu_batch_size"]),
        int(distributed["global_batch_size"]),
    )
    _expect(geometry == (2, 32, 64), f"Unexpected DDP geometry: {geometry}")
    _expect(
        int(distributed["global_batch_size"])
        == int(distributed["world_size"]) * int(distributed["per_gpu_batch_size"]),
        "global_batch_size must equal world_size * per_gpu_batch_size",
    )
    ...
    crop_widths = {
        int(key): int(value) for key, value in input_config["crop_widths"].items()
    }
    _expect(
    ...
    source_columns = {
        int(key): int(value) for key, value in resampling["source_patch_columns"].items()
    }
    target_bins = {
        int(key): int(value) for key, value in resampling["target_direction_bins"].items()
    }
    _expect(
    ...
    _expect(
        ...
        and column["satellite_sampling"]
        == "dense_center_to_square_boundary_midpoint_bilinear_assignment",
    ...
    starts = {
        int(index): int(epoch)
        for index, epoch in finetuning["block_update_start_epochs"].items()
    }
    _expect(
    ...
    _expect(
        score["name"] == "fov_masked_cyclic_hard_max"
        and score.get("shift_reduction") == "hard_max",
    ...
    _expect(per_fov == 16 and per_fov * len(fovs) == geometry[2], "Mixed batch must be 16x4")
    _expect(
        int(mixed["samples_per_fov_per_rank"]) == 8,
        "Each rank must receive 8 samples of every FoV",
    )
    ...
    _expect(
        [float(stage["hard_batch_fraction"]) for stage in stages]
    ...
    _expect(
        hard.get("pool_source") == "current_cor_geo_model"
        and "common_pool_run_root_template" not in hard,
        "Hard mining must be self-contained",
    )
    _expect(
        list(map(int, hard["refresh_after_epochs"]))
        == [16, 24, 32, 40, 48, 56],
        "Hard pools must refresh every eight epochs after epoch 16",
    )
    _expect(
        hard["candidate_strategy"]
        == "signed_dc_low2_fft_magnitude_top512_exact_cyclic_hardmax_top64_v1"
    ...
    _expect(
        ...
        and hard["anchor_selection"]
        == "globally_unique_shuffle_no_pool_exclusion"
    ...
    _expect(
        int(scheduler["content_order_encoder"]["cosine_end_global_step"])
        == 48 * 555
    ...
    _expect(
        list(map(int, config.train["checkpoint"]["retained_epochs"]))
