...
from cor_geo.models.cor_geo_model import CoRGeoOutput
...
def symmetric_cross_entropy(
    ...
    return 0.5 * (
        functional.cross_entropy(
            scores / float(temperature), targets, label_smoothing=float(label_smoothing)
        )
        + functional.cross_entropy(
            scores.T / float(temperature), targets, label_smoothing=float(label_smoothing)
        )
    )
...
class CoRGeoLoss(nn.Module):
    ...
    def forward(self, output: CoRGeoOutput) -> LossOutput:
        ...
        ground_order_direction = differentiable_all_gather(
            output.ground.order_direction
        )
        ...
        satellite_order_direction = differentiable_all_gather(
            output.satellite.order_direction
        )
        shift_logits = cyclic_direction_logits(
            ground_direction, satellite_direction, ground_valid
        )
        scores = cyclic_hard_max_score(shift_logits)
        joint_retrieval = symmetric_cross_entropy(
            scores, self.info_nce_temperature, self.label_smoothing
        )
        order_shift_logits = cyclic_direction_logits(
            ground_order_direction, satellite_order_direction, ground_valid
        )
        order_scores = cyclic_hard_max_score(order_shift_logits)
        order_retrieval = symmetric_cross_entropy(
            order_scores, self.info_nce_temperature, self.label_smoothing
        )
