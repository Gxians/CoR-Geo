...
from cor_geo.datasets.cross_view import SampleRequest
...
def stage_for_epoch(
    ...
    matches = [
        stage
        for stage in stages
        if int(stage["epoch_start"]) <= int(epoch) <= int(stage["epoch_end"])
    ]
    if len(matches) != 1:
...
def _uniform_top64_hard_subgroups(
    ...
    for anchor in anchors:
        ...
        if len(selected) < neighbors_per_anchor:
            ...
            selected.extend(
                _take_unique(fallback, neighbors_per_anchor - len(selected), used)
            )
...
def build_epoch_plan(
    ...
    if hard_count:
        ...
        for batch_index in range(hard_count):
            ...
            hard_batches.append(
                PlannedBatch("hard", indices, assignments, hard_source)
            )
...
class PlannedRankBatchSampler(Sampler[list[SampleRequest]]):
    ...
    def set_plan(self, plan: Sequence[PlannedBatch], epoch: int) -> None:
        ...
        if any(
            len(batch.indices) != expected or len(batch.fov_degrees) != expected
            for batch in plan
        ):
    ...
    def __iter__(self) -> Iterator[list[SampleRequest]]:
        ...
        for batch in self.plan:
            ...
            counts = {
                fov: sum(request.fov_deg == fov for request in requests)
