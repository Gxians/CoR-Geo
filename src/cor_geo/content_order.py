"""Column/ray construction and shared Content-Order direction encoding."""
...
class ConservativeAngularResampler(nn.Module):
    ...
    def forward(self, features: Tensor) -> Tensor:
        ...
        if features.ndim != 4 or features.shape[2] != self.source_bins:
            raise ValueError(f"Expected BH{self.source_bins}C source features, got {tuple(features.shape)}")
...
class BilinearSquareRaySampler(nn.Module):
    ...
    def __init__(
        ...
        theta = torch.arange(angular_bins, dtype=torch.float64) * (2.0 * math.pi / angular_bins)
        sin_theta = theta.sin()
        cos_theta = theta.cos()
        ...
        fraction = (torch.arange(sequence_length, dtype=torch.float64) + 0.5) / sequence_length
    ...
    def forward(self, features: Tensor) -> Tensor:
        ...
        if (height, width) != (self.input_size, self.input_size):
            raise ValueError(
                f"Satellite token grid must be {self.input_size}x{self.input_size}, " f"got {height}x{width}"
            )
        assignment = self.assignment.to(device=features.device, dtype=features.dtype)
...
class ContentOrderEncoder(nn.Module):
    ...
    def forward(
        ...
        if sequences.ndim != 4 or sequences.shape[-1] != self.input_dim:
            raise ValueError(f"Expected [B,A,L,{self.input_dim}] sequences, got {tuple(sequences.shape)}")
        projected = self.projection(sequences)
        content_tokens, order_tokens = projected.split((self.content_dim, self.order_dim), dim=-1)
        logits = torch.einsum(
            "bald,d->bal",
        ...
        content_raw = torch.einsum("bal,bald->bad", attention.to(content_tokens.dtype), content_tokens)
        order_weights = self.first_cosine_weights(sequences.shape[2], sequences.device).to(order_tokens.dtype)
        ...
        direction = torch.cat((content * content_scale, order * order_scale), dim=-1)
